...
%*--------------------------------------------------------------------------------*
 ...
 |                                                                                |
 | SYNTAX:  CALL COMPLEXADD (augend, addend, sum)                                 |
...
proc fcmp outlib=library.functions.smd ;
   deletesubr complexadd ;
run ;

proc fcmp outlib=library.functions.smd ;
   subroutine complexadd(augend[2], addend[2], sum[2]) ;
      outargs sum ;
      static err ;
      err = 0 ;
      ...
      if not (dim(augend)=dim(addend)=dim(sum)=2) then do ;
         if err = 0 then do ;
            put 'NOTE: Array subscript out of range in CALL COMPLEXADD, complex numbers (arrays) must have the two dimensions.' ;
            err = 1 ;
         end ;
         sum[1] = . ;
         sum[2] = . ;
         ...
         if err = 0 then do ;
            put 'NOTE: Missing values were generated as a result of performing a complex number addition operation on missing values in CALL COMPLEXADD.' ;
            err = 1 ;
         end ;
         sum[1] = . ;
         sum[2] = . ;
