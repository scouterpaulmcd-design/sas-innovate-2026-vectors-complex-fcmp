...
%*--------------------------------------------------------------------------------*
 | CALL:     vectorcopyc.sas                                                      |
 |                                                                                |
 | PURPOSE:  This subroutine copies all members of a character SAS array from one |
 |           array to another.  Although this call does not apply to the use of   |
 |           vectors and complex numbers, we added it for completeness as a       |
 |           character version of the CALL VECTORCOPYN routine.                   |
 |                                                                                |
 | SYNTAX:   CALL VECTORCOPYC (from, to)                                          |
 |            from = character-SAS-array [required] A character SAS variable      |
 |                   array to copy.  Arrays from and to must be the same          |
 |                   dimension.                                                   |
 |            to   = character-SAS-array [required] A character SAS variable      |
 |                   array to serve as the destination array that is to be        |
...
proc fcmp outlib=library.functions.smd ;
   deletesubr vectorcopyc ;
run ;

proc fcmp outlib=library.functions.smd ;
   subroutine vectorcopyc(from[*] $, to[*] $) ;
      outargs to ;
      static err 0 ;
      length i 8 ;
      ...
      if dim(from) = dim(to) then do ;
         ...
         if err = 0 then do ;
            put 'NOTE: Array subscript out of range in CALL VECTORCOPYC, vectors (arrays) must have the same dimensions.' ;
            err = 1 ;
         end ;
         call vectorinitc(to, '') ;
      end ;
...
data _null_ ;
   ...
   call vectorinitc (a, '1') ;

   call vectorcopyc(a, b) ;
   put b1= b2= b3= ;
run ;
*/
